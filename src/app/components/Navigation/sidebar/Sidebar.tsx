...
const Sidebar = () => {
  return (
    <motion.div
      className="fixed w-full top-0 right-0 h-screen md:w-96 bg-neutral-900 z-20 pt-8 pb-16 px-20 flex flex-col justify-between dark:bg-white"
    ...
    >
      <ModeToggler />
      <div>
        <p className="text-gray-400 font-serif mb-3 dark:text-gray-700">
          Navigation
        </p>
        ...
        <ul>
          {routesMapping.map((route, index) => (
            <li key={index} className="mb-7">
              <Link
                className="text-white text-3xl font-serif hover:underline dark:text-black"
                href={route.path}
              >
                {route.name}
              </Link>
